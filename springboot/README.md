...
__Client Tier__
This tier is for the consumer of this application. It could either be a trader who is capable of using the REST API or some endpoints. It can be a Swagger UI, Postman or even using `curl` command from the Linux system. The HTTP request from this tierwill be received and handled by the Tomcat Servlet. After the information is processed, the servelet will send HTTP responses back to the client. And it will be the client's responsibility to format the HTTP responses.
